...
export async function fetchPMaxCampaigns() {
  ...
  const gaqlQuery = `
    SELECT campaign.id, campaign.name FROM campaign WHERE campaign.advertising_channel_type = 'PERFORMANCE_MAX' AND campaign.status = 'ENABLED' ORDER BY campaign.name
  `;
